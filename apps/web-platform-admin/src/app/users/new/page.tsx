...
function extractProblemDetails(error: unknown): ProblemDetails | null {
	...
	const hasAnyKnownKey = ["title", "detail", "status", "errors"].some(
		(key) => key in maybe,
	);
	if (!hasAnyKnownKey) {
		return null;
	...
	const errors =
		typeof maybe.errors === "object" && maybe.errors !== null
			? (maybe.errors as Record<string, unknown>)
			: undefined;
...
export default function CreateUserPage() {
	...
	const onSubmit = form.handleSubmit(async (values) => {
		...
		try {
			...
			const message =
				problem?.detail || problem?.title || "Failed to create user";
			toast.error(message);
			...
			<div className="mb-6 flex flex-col gap-2">
				<div>
					...
					<p className="text-muted-foreground mt-2">
						Provide the user details to invite them into the workspace.
					</p>
				</div>
				{submitError && (
