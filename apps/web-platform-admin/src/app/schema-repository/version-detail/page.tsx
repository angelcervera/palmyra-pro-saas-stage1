...
export default function SchemaVersionDetailPage() {
	...
	const { data, isLoading, isError, error } = useSchemaVersion(
		schemaId,
		schemaVersion,
	);
		...
		if (isError) {
			const message =
				error instanceof Error
					? error.message
					: "Unable to load the schema version. Please try again.";
			toast.error(message);
		}
	}, [isError, error]);
			...
			<div className="flex items-center justify-between">
				...
				<div className="flex items-center gap-2">
					...
					<Button asChild variant="outline" disabled={!data || isLoading}>
						<Link
							to={`/schema-repository/${schemaId}/versions/${schemaVersion}/edit`}
						>
							Edit definition
						</Link>
					</Button>
			...
			<Card>
				<CardHeader className="gap-2">
					<CardTitle>
						...
						{data?.isActive ? (
							<Badge className="ml-2 bg-emerald-100 text-emerald-900 hover:bg-emerald-200">
								Active
							</Badge>
						) : (
							<Badge
					...
					<CardDescription>
						Metadata persisted in the schema repository.
					</CardDescription>
				</CardHeader>
					...
					{data && (
						<div className="space-y-4">
							<dl className="grid gap-4 sm:grid-cols-2">
								...
								<MetadataItem
									label="Active"
									value={formatBoolean(data.isActive)}
								/>
								<MetadataItem
									label="Soft deleted"
