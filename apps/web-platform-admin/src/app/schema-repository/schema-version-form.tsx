...
export function SchemaVersionForm({
	...
	const mergedDefaults = useMemo<SchemaVersionFormValues>(
		() => ({
			...
			schemaDefinition:
				defaultValues?.schemaDefinition ?? DEFAULT_SCHEMA_TEMPLATE,
		}),
		[defaultValues],
	);
	...
	const handleSubmit = form.handleSubmit(
		async (values) => {
			try {
				const parsed = JSON.parse(values.schemaDefinition) as Record<
					string,
					unknown
				>;
				if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
						...
						render={({ field }) => (
							<FormItem>
								...
								<Select
									...
									disabled={
										categories.length === 0 || Boolean(readOnlyFields?.category)
									}
								>
									<FormControl>
										<SelectTrigger
  ...
  "properties": {}
								...
								<FormDescription>
									Provide a valid JSON Schema object. This value is persisted
									as-is.
								</FormDescription>
								<FormMessage />
							</FormItem>
