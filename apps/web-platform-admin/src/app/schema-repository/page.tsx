		...
		if (isError) {
			const message =
				error instanceof Error
					? error.message
					: "Failed to load schema versions. Please try again.";
			toast.error(message);
		}
	}, [isError, error]);

	const versions = useMemo(() => {
		const items = (data ?? []).filter((version) =>
			!onlyActive ? true : version.isActive,
		);
			...
			<div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
				<div>
					...
					<p className="text-muted-foreground text-sm">
						Inspect and manage every JSON schema version stored in the
						persistent layer.
					</p>
				</div>
