...
import { useSchemaCategory, useUpdateSchemaCategory } from '../use-schema-categories'
    ...
    <div className="flex flex-1 flex-col gap-6 p-6">
      ...
      {isLoading ? (
        ...
        <div className="rounded-md border border-destructive/30 bg-destructive/10 p-4 text-sm text-destructive">
          Unable to load schema category. {error instanceof Error ? error.message : 'Please try again.'}
        </div>
      ) : (
        <Card>
          <CardHeader>
            <CardTitle>Category details</CardTitle>
            <CardDescription>Adjust the slug, name, description, or parent relationship.</CardDescription>
          </CardHeader>
          <CardContent>
            <SchemaCategoryForm
              defaultValues={{
