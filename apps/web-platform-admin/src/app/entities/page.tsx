...
export default function EntitiesPage() {
	...
	const fetchTables = useCallback(async () => {
		...
		try {
			...
			const payload =
				raw as SchemaRepository.ListAllSchemaVersionsResponses[200];
	...
	const hasChanges = useMemo(() => {
		return rows.some((row) => {
			const entityId =
				typeof row.entityId === "string" ? row.entityId : undefined;
			...
			<Card>
				<CardHeader className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
					<div>
						...
						<CardDescription>
							Choose an active table to load documents, then edit inline and
							save.
						</CardDescription>
					</div>
					<div className="flex items-center gap-2">
						...
						<Button
							size="sm"
							onClick={handleSave}
							disabled={saving || !hasChanges || !tableName}
						>
							{saving ? "Saving…" : "Save"}
						</Button>
					</div>
