...
import { SchemaCategoryForm, type SchemaCategoryFormValues } from '../schema-category-form'
  ...
  const handleSubmit = useCallback(
    async (values: SchemaCategoryFormValues) => {
      try {
        ...
        const message =
          error instanceof Error ? error.message : 'Failed to create schema category. Please try again.'
        toast.error(message)
      }
    },
    ...
    <div className="flex flex-1 flex-col gap-6 p-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Provide the metadata that identifies this schema category.</CardDescription>
        </CardHeader>
        <CardContent>
          <SchemaCategoryForm isSubmitting={createMutation.isPending} onSubmit={handleSubmit} />
        </CardContent>
      </Card>
    </div>
  )
