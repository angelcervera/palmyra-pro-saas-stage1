...
import { useDeleteSchemaCategory, useSchemaCategories } from './use-schema-categories'
...
export default function SchemaCategoriesPage() {
  ...
  useEffect(() => {
    if (isError) {
      const message = error instanceof Error ? error.message : 'Failed to load schema categories.'
      toast.error(message)
    }
  }, [isError, error])
    ...
    <div className="flex flex-1 flex-col p-6">
      {isError ? (
        <div className="rounded-md border border-destructive/30 bg-destructive/10 p-4 text-sm text-destructive">
          Failed to load schema categories. {error instanceof Error ? error.message : 'Please try again.'}
        </div>
      ) : (
