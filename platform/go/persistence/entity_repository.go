...
func (r *EntityRepository) UpdateEntity(ctx context.Context, params UpdateEntityParams) (EntityRecord, error) {
	if params.EntityID == uuid.Nil {
		return EntityRecord{}, errors.New("entity id is required")
	}
	if len(params.Payload) == 0 {
		return EntityRecord{}, errors.New("payload is required")
	}
	...
	selectStmt := fmt.Sprintf(`
		...
		WHERE entity_id = $1 AND entity_version = $2
	`, r.tableIdent)
	row := tx.QueryRow(ctx, selectStmt, params.EntityID, nextVersion.String())
	record, err := scanEntityRecord(row)
	if err != nil {
...
func (r *EntityRepository) CreateOrUpdateEntity(ctx context.Context, params CreateOrUpdateEntityParams) (EntityRecord, error) {
	...
	// No entity ID provided – treat as create with autogenerated identifier.
	if params.EntityID == uuid.Nil {
		if params.Slug == nil {
...
func (r *EntityRepository) GetEntityByID(ctx context.Context, entityID uuid.UUID) (EntityRecord, error) {
	if entityID == uuid.Nil {
		return EntityRecord{}, errors.New("entity id is required")
	}
...
func (r *EntityRepository) GetEntityVersion(ctx context.Context, entityID uuid.UUID, version SemanticVersion) (EntityRecord, error) {
	...
	row := r.pool.QueryRow(ctx, query, entityID, version.String())
	record, err := scanEntityRecord(row)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return EntityRecord{}, ErrEntityNotFound
