...
export class PersistenceClient implements PersistenceProvider {
    ...
    async getEntity<TPayload = unknown>(ref: EntityIdentifier): Promise<EntityRecord<TPayload>> {
    ...
    async saveEntity<TPayload = unknown>(input: SaveEntityInput<TPayload>): Promise<EntityRecord<TPayload>> {
