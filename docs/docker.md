...
2. `apps/web-platform-admin/Dockerfile` — multi-stage build for the React admin UI. It installs the monorepo with pnpm, builds the Vite bundle, and publishes the compiled assets behind an Nginx proxy that forwards `/api/*` calls to the Go API.
...
| Scenario                                   | Use                               | Notes                                                                                                                                                                                                |
|--------------------------------------------|-----------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Local development (API + DB + Admin UI)    | `docker compose up --build`       | Reads configuration from `.env.dockercompose`. Spins up `postgres`, `api`, and `web-platform-admin` containers. Auth middleware defaults to `AUTH_PROVIDER=dev`, and the `web-platform-admin` service proxies `/api/*` to the API. |
...
| Deploying static admin UI (Cloud storage)  | Build from `apps/web-platform-admin/Dockerfile` | Publishes a ready-to-serve bundle. Provide a reverse proxy (or CDN) that forwards `/api/*` to the backend service, matching what the Nginx stage does locally.                                          |
...
3. Once the stack is up:
   - Admin UI: `http://localhost:4173` (served by the `web-platform-admin` container). Requests to `/api/*` are transparently proxied to the Go API service.
...
4. Tear everything down when done:
   ...
   docker compose down
   ```
5. Optional: if you still prefer hot-reload development, you can run `pnpm dev -C apps/web-platform-admin` locally and skip the `web-platform-admin` container. Compose continues to provide Postgres + API for that workflow.
...
- To inspect logs: `docker compose logs -f api`, `docker compose logs -f web-platform-admin`, or `docker compose logs -f postgres`.
